...
def write_summary_charts(rows: list[dict], destination: Path) -> list[str]:
    ...
    l1 = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("hypothesis:N", sort=None, title="Hypothesis"),
            y=alt.Y("l1:Q", title="L1 abundance error"),
            color=alt.Color("dataset:N", title="Dataset"),
            xOffset="dataset:N",
            tooltip=["dataset", "hypothesis", "l1", "f1", "accuracy"],
        )
        .properties(title="Abundance error by colouring and resolution hypothesis", width=460, height=280)
    )
    f1 = (
        alt.Chart(frame)
        .mark_bar()
        ...
        .properties(title="Species assignment by hypothesis", width=460, height=280)
    )
