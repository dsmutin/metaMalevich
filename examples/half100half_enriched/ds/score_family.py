"""Family scores for the 10x-depth half100half assembly.
...
Unclassified reads inherit a contig family from neighbour agreement on the
...
EXAMPLE = ROOT / "examples" / "half100half_enriched"
WORK = EXAMPLE / "work"
...
def _truth() -> dict[int, float]:
    accessions = {
        row["accession"]: int(row["family_taxid"])
...
def _profile(reads: dict[str, int], placement: dict[str, str], labels: dict[str, int] | None, parser) -> dict[int, float]:
    ...
    for read_id, taxon_id in reads.items():
        family_id = _family(parser, taxon_id)
        if labels is not None and family_id == 0:
            contig_id = placement.get(read_id)
            if contig_id is not None:
                family_id = labels.get(contig_id, 0)
        counts[family_id] += 1
    total = sum(counts.values())
    return {taxon_id: count / total for taxon_id, count in counts.items()}
...
def main() -> None:
    ...
    final_labels = {node_id: _family(parser, final_calls.get(node_id, 0)) for node_id in final_sequences}
    initial_labels = {node_id: _family(parser, initial_calls.get(node_id, 0)) for node_id in initial_sequences}
    ...
    write_tsv(destination / "family_metrics.tsv", rows, list(rows[0].keys()))
